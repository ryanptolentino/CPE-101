...
class TestData(unittest.TestCase):
    ...
    def test_sphere_1(self):
        pt1 = data.Point(1, 2, 3)
        sphere1 = data.Sphere(pt1, 3.5, data.Color(255, 255, 255), 1)
        self.assertTrue(sphere1.center == pt1)
        self.assertEqual(sphere1.radius, 3.5)
        self.assertEqual(sphere1.color, data.Color(255, 255, 255))
        self.assertEqual(sphere1.finish, 1)

    def test_sphere_2(self):
        pt2 = data.Point(4, 5, 6)
        sphere2 = data.Sphere(pt2, 4.5, data.Color(255, 255, 255), 3)
        self.assertTrue(sphere2.center == pt2)
        self.assertEqual(sphere2.radius, 4.5)
        self.assertEqual(sphere2.color, data.Color(255, 255, 255))
        self.assertEqual(sphere2.finish, 3)
    ...
    def test_sphere_equals(self):
        ...
        sphere1 = data.Sphere(pt1, 3.5, data.Color(255, 255, 255), finish)
        sphere2 = data.Sphere(pt2, 3.5, data.Color(255, 255, 255), finish)
        sphere3 = data.Sphere(pt3, 3.5, data.Color(255, 255, 255), finish)
        sphere4 = data.Sphere(pt1, 6.5, data.Color(255, 255, 255), finish)
    ...
    def test_sphere_intersection_point(self):
        ...
        sphere1 = data.Sphere(pt2, 1, data.Color(255, 255, 255), 1)
    ...
    def test_find_intersection_points(self):
        ...
        sphere2 = data.Sphere(pt3, 1, data.Color(255, 255, 255), 1)  # does not intersect with this sphere instance

        pt4 = data.Point(8, 6, 0)
        sphere3 = data.Sphere(pt4, 1, data.Color(255, 255, 255), 1)
    ...
    def test_sphere_normal_at(self):
        pt1 = data.Point(0, 0, 0)
        sphere = data.Sphere(pt1, 2, data.Color(255, 255, 255), 1)
        spherePoint = data.Point(0, 2, 0)
        normalVector = data.Vector(0, 1, 0)
        self.assertEqual(collisions.sphere_normal_at_point(sphere, spherePoint), normalVector)

    def test_sphere_normal_at2(self):
        pt1 = data.Point(0, 0, 0)
        sphere = data.Sphere(pt1, 1, data.Color(255, 255, 255), 1)
    ...
    def test_cast_ray(self):
        ...
        sphere1 = data.Sphere(data.Point(1.0, 1.0, 0.0), 2.0, data.Color(0.0, 0.0, 1.0), finish1)
        sphere2 = data.Sphere(data.Point(0.5, 1.5, -3.0), 0.5, data.Color(1.0, 0.0, 0.0), finish2)
        sphere_list = [sphere1, sphere2]

        self.assertEqual(cast.cast_ray(r1, sphere_list, ambient, light, eye_point), data.Color(0.0, 0.0, 0.2))
